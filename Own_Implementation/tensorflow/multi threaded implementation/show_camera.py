...
class Show_Camera(threading.Thread):
    ...
    def run(self):
           ...
           # print (self.shared_variables.name)
            if self.shared_variables.camera_capture.isOpened():
                ...
                if self.show_fps:
                    ...
                    cv2.putText(self.frame, str(fps), (0, 100), font, 1, (255, 255, 255), 1, cv2.LINE_AA)


                # show frame
                cv2.imshow('Tracking And Detection', self.frame)
                if cv2.waitKey(1) == 27:
                    break  # esc to quit
        ...
        self.shared_variables.running = False
